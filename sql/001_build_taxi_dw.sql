-- Taxi rides warehouse incremental build/load (Databricks SQL)
-- Source: samples.nyctaxi.trips

CREATE CATALOG IF NOT EXISTS main;
CREATE SCHEMA IF NOT EXISTS main.taxi_dw;

-- Control/watermark table for incremental processing.
...
-- Optional external ZIP reference table (maintained by notebook pipeline).
...
-- Incremental slice with 2-day lookback to capture late arrivals safely.
-- This stays idempotent because downstream loads use MERGE keys.
...
-- Advance watermark after successful merges.
MERGE INTO main.taxi_dw.etl_watermark t
USING (
  SELECT
    'fact_taxi_rides' AS process_name,
